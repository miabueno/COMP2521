...
static Node doInsert(Node n, Time time) {
    ...
    int rebalance = height(n->left) - height(n->right);
    
    if (rebalance > 1) {
        cmp = TimeCmp(time, n->left->time);
        if (cmp < 0) n = rotateRight(n);
        else {
...
static Node rotateRight(Node n) {
    ...
    m->height = 1 + max(height(m->right), height(m->left));
...
static Node findCeiling(Node n, Time time) {
    ...
    int cmp = TimeCmp(n->time, time);
  
    // equal
    if (cmp == 0) return n; 
  
    // If root's time is before, ceil must be in right subtree 
    if (cmp < 0) return findCeiling(n->right, time); 
  
    // Else, either left subtree or root has the ceil value 
    Node ceil = findCeiling(n->left, time); 
    return (ceil != NULL) ? ceil : n; 
}
...
static Node findFloor(Node n, Time time) {
    ...
    if (cmp > 0) return findFloor(n->left, time);
    
    Node floor = findFloor(n->right, time);
    return (floor != NULL) ? floor : n;
}
