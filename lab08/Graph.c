...
int * solve(Graph g, Vertex src, int max);
int * reconstructPath(Vertex src, Vertex dest, int *prev);
...
int findPath (Graph g, Vertex src, Vertex dest, int max, int *path)
{
	...
	int *prev = solve(g, src, max); // Do a BFS starting at Vertex src
	
	path = reconstructPath(src,dest,prev);
    ...
    while (!found && !QueueIsEmpty(q)) {
        ...
        if (v == dest) found = 1;
            ...
            for (int w = 0; w < g->nV; w++) {
                ...
                if (!visited[w] && g->edges[v][w] > 0 && g->edges[v][w] < max) {
...
int * reconstructPath(Vertex src, Vertex dest, int *prev) {
    ...
    // Reconstruct path going backwards from dest
    int *path;
    for (int i = dest; i != -1; i = prev[i]) path[i + 1] = i; //unsure
